$(document).ready(function() {      
    var coreJSON,abouttrust_id ,mode;
...
// *************************** [Validation] ******************************************************************** 
    $('#formSubmit').click(function(){
        ...
        if(aboutdescription.getData() == '')
        {
            $('.about_description').html('Please Enter The abouttrust Detail Description');
            $('.about_description').show();
        }
        // else if($('#image_url').val()=="" && mode == 'new')
        // {
        //     $('.image_url').html('Please choose abouttrust image');
        //     $('.image_url').show();
        // }
    ...
    {       
        ...
        data.append("about_description",aboutdescription.getData());
        if (mode == "new")
        {
            var url = base_URL+"admin/KaanalAdmin/insert_abouttrust";
        }
        else if (mode == "update")
        {
            var url = base_URL+"admin/KaanalAdmin/update_abouttrust";
            data.append("abouttrust_id",abouttrust_id);
        }
        request = $.ajax({
                type: "POST",
    ...
    {
        return $.ajax({
            url: base_URL+'admin/KaanalAdmin/get_abouttrust',
            type:'POST',
            success:function(data){
...
// *************************** [Display] ********************************************************************
 ...
 $(document).on('change', '#image_url', function() {
    ...
    $(document).on('click', '.btnEdit', function() {
        ...
        mode = "update";
        $('#myModal').modal('show');
        // $('#abouttrust_title').val(coreJSON[r_index].abouttrust_title);
        aboutdescription.setData(coreJSON[r_index].about_description);
        $('#user_uploaded_image').attr("src", base_URL+coreJSON[r_index].image_url);
        $("#user_uploaded_image").addClass("active");
        $("#user_uploaded_image").css("display", "block");
        abouttrust_id = coreJSON[r_index].abouttrust_id;
    });
    ...
    $(document).on('click','.BtnDelete',function(){
        ...
        Swal.fire({
            ...
            $.ajax({
                type: "POST",
                url: base_URL+'admin/KaanalAdmin/delete_abouttrust',
                data: {"abouttrust_id":abouttrust_id },
                success: function (data) {
                    var insertResult = $.parseJSON(data);
                    if(insertResult.status=="success")
                    {

$(document).ready(function() {      
    var coreJSON,representatives_id ,mode;
...
// *************************** [Validation] ******************************************************************** 
    $('#formSubmit').click(function(){
        ...
        if($('#place').val()=="" && mode == 'new')
        {
            $('.place').html('Please Enter Place');
            $('.place').show();
        }
        // if($('#position').val()=="" && mode == 'new')
        // {
        //     $('.position').html('Please Enter position');
        //     $('.position').show();
        // }
        else if($('#image_url').val()=="" && mode == 'new')
        {
            $('.image_url').html('Please Trust Representatives image');
            $('.image_url').show();
        }
    ...
    {       
        ...
        if (mode == "new")
        {
            var url = base_URL+"admin/KaanalAdmin/insert_representatives";
        }
        else if (mode == "update")
        {
            var url = base_URL+"admin/KaanalAdmin/update_representatives";
            data.append("representatives_id",representatives_id);
        }
        request = $.ajax({
                type: "POST",
    ...
    {
        return $.ajax({
            url: base_URL+'admin/KaanalAdmin/get_representatives',
            type:'POST',
            success:function(data){
...
// *************************** [Display] ********************************************************************
 ...
 $(document).on('change', '#image_url', function() {
    ...
    $(document).on('click', '.btnEdit', function() {
        ...
        $('#place').val(coreJSON[r_index].place);
        $('#position').val(coreJSON[r_index].position);
        $('#user_uploaded_image').attr("src", base_URL+coreJSON[r_index].image_url);
        $("#user_uploaded_image").addClass("active");
        $("#user_uploaded_image").css("display", "block");
        representatives_id = coreJSON[r_index].representatives_id;
    });
    ...
    $(document).on('click','.BtnDelete',function(){
        ...
        Swal.fire({
            ...
            $.ajax({
                type: "POST",
                url: base_URL+'admin/KaanalAdmin/delete_representatives',
                data: {"representatives_id":representatives_id },
                success: function (data) {
                    var insertResult = $.parseJSON(data);
                    if(insertResult.status=="success")
                    {

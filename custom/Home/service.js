...
// *************************** [Validation] ******************************************************************** 
    $('#formSubmit').click(function(){
        $('.invalid-feedback').hide();
         if($('#service_title').val()=="" && mode == 'new')
        {
            $('.service_title').html('Please choose Service Title');
            $('.service_title').show();
        }
      
       else if(serviceDescription.getData() == '')
        {
            $('.service_description').html('Please Enter The Service Detail Description');
            $('.service_description').show();
        }
        else if($('#image_url').val()=="" && mode == 'new')
        {
            $('.image_url').html('Please choose Service image');
            $('.image_url').show();
        }
    ...
    {       
        ...
        data.append("service_description",serviceDescription.getData());
        if (mode == "new")
        {
            var url = base_URL+"admin/KaanalAdmin/insert_service";
        }
        else if (mode == "update")
        {
            var url = base_URL+"admin/KaanalAdmin/update_service";
    ...
    {
        return $.ajax({
            url: base_URL+'admin/KaanalAdmin/get_service',
            type:'POST',
            success:function(data){
...
// *************************** [Display] ********************************************************************
 ...
 $(document).on('change', '#image_url', function() {
    ...
    $(document).on('click', '.btnEdit', function() {
        ...
        mode = "update";
        $('#myModal').modal('show');
        $('#service_title').val(coreJSON[r_index].service_title);
        serviceDescription.setData(coreJSON[r_index].service_description);
        $('#user_uploaded_image').attr("src", base_URL+coreJSON[r_index].image_url);
        $("#user_uploaded_image").addClass("active");
        $("#user_uploaded_image").css("display", "block");
        service_id = coreJSON[r_index].service_id;
    });
    ...
    $(document).on('click','.BtnDelete',function(){
        ...
        Swal.fire({
            ...
            $.ajax({
                type: "POST",
                url: base_URL+'admin/KaanalAdmin/delete_service',

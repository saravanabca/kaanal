...
// *************************** [Validation] ******************************************************************** 
    $('#formSubmit').click(function(){
        ...
        if(partnersdescription.getData() == '')
        {
            $('.partners_description').html('Please Enter The partners Detail Description');
            $('.partners_description').show();
        }
        else if($('#image_url').val()=="" && mode == 'new')
        {
            $('.image_url').html('Please choose partners image');
            $('.image_url').show();
        }
    ...
    {       
        ...
        data.append("partners_description",partnersdescription.getData());
        if (mode == "new")
        {
            var url = base_URL+"admin/KaanalAdmin/insert_partners";
        }
        else if (mode == "update")
        {
            var url = base_URL+"admin/KaanalAdmin/update_partners";
            data.append("partners_id",partners_id);
        }
        request = $.ajax({
                type: "POST",
    ...
    {
        return $.ajax({
            url: base_URL+'admin/KaanalAdmin/get_partners',
            type:'POST',
            success:function(data){
...
// *************************** [Display] ********************************************************************
    function dispGallery(JSON)
    {
       ...
       $('#datatable').dataTable( {
            ...
            "aoColumns": [
                ...
                { 
                    "mDataProp": function ( data, type, full, meta) {
                        return data.partners_description;
 ...
 $(document).on('change', '#image_url', function() {
    ...
    $(document).on('click', '.btnEdit', function() {
        ...
        mode = "update";
        $('#myModal').modal('show');
        // $('#partners_title').val(coreJSON[r_index].partners_title);
        partnersdescription.setData(coreJSON[r_index].partners_description);
        $('#user_uploaded_image').attr("src", base_URL+coreJSON[r_index].image_url);
        $("#user_uploaded_image").addClass("active");
        $("#user_uploaded_image").css("display", "block");
        partners_id = coreJSON[r_index].partners_id;
    });
    ...
    $(document).on('click','.BtnDelete',function(){
        ...
        Swal.fire({
            ...
            $.ajax({
                type: "POST",
                url: base_URL+'admin/KaanalAdmin/delete_partners',
                data: {"partners_id":partners_id },
                success: function (data) {
                    var insertResult = $.parseJSON(data);
                    if(insertResult.status=="success")
                    {

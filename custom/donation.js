...
// *************************** [Validation] ******************************************************************** 
    $('#formSubmit').click(function(){
        ...
        if(Donation.getData() == '')
        {
            $('.donation').html('Please Enter The Donation');
            $('.donation').show();
        }
    ...
    {       
        ...
        data.append("donation",Donation.getData());
        if (mode == "new")
        {
            var url = base_URL+"admin/KaanalAdmin/insert_donation";
        }
        else if (mode == "update")
        {
            var url = base_URL+"admin/KaanalAdmin/update_donation";
    ...
    {
        return $.ajax({
            url: base_URL+'admin/KaanalAdmin/get_donation',
            type:'POST',
            success:function(data){
...
// *************************** [Display] ********************************************************************
 ...
 $(document).on('change', '#image_url', function() {
    ...
    $(document).on('click', '.btnEdit', function() {
        ...
        mode = "update";
        $('#myModal').modal('show');
        // $('#donation_title').val(coreJSON[r_index].donation_title);
        Donation.setData(coreJSON[r_index].donation);
        $('#user_uploaded_image').attr("src", base_URL+coreJSON[r_index].image_url);
        $("#user_uploaded_image").addClass("active");
        $("#user_uploaded_image").css("display", "block");
        donation_id = coreJSON[r_index].donation_id;
    });
    ...
    $(document).on('click','.BtnDelete',function(){
        ...
        Swal.fire({
            ...
            $.ajax({
                type: "POST",
                url: base_URL+'admin/KaanalAdmin/delete_donation',

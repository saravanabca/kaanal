$(document).ready(function() {      
    var coreJSON,newstitle_id ,mode;
...
// *************************** [Validation] ******************************************************************** 
    $('#formSubmit').click(function(){
        $('.invalid-feedback').hide();
         ...
         if($('#newstitle').val()=="" && mode == 'new')
        {
            $('.newstitle').html('Please Enter News Title');
            $('.newstitle').show();
        }
        else
        {
    ...
    {       
        ...
        // data.append("newstitle_description",newstitledescription.getData());
        if (mode == "new")
        {
            var url = base_URL+"admin/KaanalAdmin/insert_newstitle";
        }
        else if (mode == "update")
        {
            var url = base_URL+"admin/KaanalAdmin/update_newstitle";
            data.append("newstitle_id",newstitle_id);
        }
        request = $.ajax({
                type: "POST",
    ...
    {
        return $.ajax({
            url: base_URL+'admin/KaanalAdmin/get_newstitle',
            type:'POST',
            success:function(data){
...
// *************************** [Display] ********************************************************************
 ...
 $(document).on('change', '#image_url', function() {
    ...
    $(document).on('click', '.btnEdit', function() {
        ...
        mode = "update";
        $('#myModal').modal('show');
        $('#newstitle').val(coreJSON[r_index].newstitle);
        

        // newstitledescription.setData(coreJSON[r_index].newstitle_description);
        // $('#user_uploaded_image').attr("src", base_URL+coreJSON[r_index].image_url);
        // $("#user_uploaded_image").addClass("active");
        // $("#user_uploaded_image").css("display", "block");
        newstitle_id = coreJSON[r_index].newstitle_id;
    });
    ...
    $(document).on('click','.BtnDelete',function(){
        ...
        Swal.fire({
            ...
            $.ajax({
                type: "POST",
                url: base_URL+'admin/KaanalAdmin/delete_newstitle',
                data: {"newstitle_id":newstitle_id },
                success: function (data) {
                    var insertResult = $.parseJSON(data);
                    if(insertResult.status=="success")
                    {

$(document).ready(function() {      
    var coreJSON,orangehvcgallery_id ,mode;
...
// *************************** [Validation] ******************************************************************** 
    $('#formSubmit').click(function(){
        ...
        if($('#orangehvc_id').val()=="" && mode == 'new')
        {
            $('.orangehvc_id').html('Please choose orangehvc  Title');
            $('.orangehvc_id').show();
        }
        else if($('#image_url').val()=="" && mode == 'new')
        {
            $('.image_url').html('Please choose gallery image');
            $('.image_url').show();
        }
    ...
    {       
        ...
        if (mode == "new")
        {
            var url = base_URL+"admin/KaanalAdmin/insert_orangehvcgallery";
        }
        else if (mode == "update")
        {
            var url = base_URL+"admin/KaanalAdmin/update_orangehvcgallery";
            data.append("orangehvcgallery_id",orangehvcgallery_id);
        }
        request = $.ajax({
                type: "POST",
    ...
    {
        return $.ajax({
            url: base_URL+'admin/KaanalAdmin/get_orangehvcgallery',
            type:'POST',
            success:function(data){
...
// *************************** [Display] ********************************************************************
 ...
 $(document).on('change', '#image_url', function() {
    ...
    $(document).on('click', '.btnEdit', function() {
        ...
        mode = "update";
        $('#myModal').modal('show');
        $('#orangehvc_id').val(coreJSON[r_index].orangehvc_id);
        $('#user_uploaded_image').attr("src", base_URL+coreJSON[r_index].image_url);
        $("#user_uploaded_image").addClass("active");
        $("#user_uploaded_image").css("display", "block");
        orangehvcgallery_id = coreJSON[r_index].orangehvcgallery_id;
    });
    ...
    $(document).on('click','.BtnDelete',function(){
        ...
        Swal.fire({
            ...
            $.ajax({
                type: "POST",
                url: base_URL+'admin/KaanalAdmin/delete_orangehvcgallery',
                data: {"orangehvcgallery_id":orangehvcgallery_id },
                success: function (data) {
                    var insertResult = $.parseJSON(data);
                    if(insertResult.status=="success")
                    {

...
// *************************** [Validation] ******************************************************************** 
    $('#formSubmit').click(function(){
        ...
        if(appealdescription.getData() == '')
        {
            $('.appeal_description').html('Please Enter The appeal Detail Description');
            $('.appeal_description').show();
        }
        // else if($('#image_url').val()=="" && mode == 'new')
        // {
        //     $('.image_url').html('Please choose appeal image');
        //     $('.image_url').show();
        // }
    ...
    {       
        ...
        data.append("appeal_description",appealdescription.getData());
        if (mode == "new")
        {
            var url = base_URL+"admin/KaanalAdmin/insert_appeal";
        }
        else if (mode == "update")
        {
            var url = base_URL+"admin/KaanalAdmin/update_appeal";
            data.append("appeal_id",appeal_id);
        }
        request = $.ajax({
                type: "POST",
    ...
    {
        return $.ajax({
            url: base_URL+'admin/KaanalAdmin/get_appeal',
            type:'POST',
            success:function(data){
...
// *************************** [Display] ********************************************************************
    function dispGallery(JSON)
    {
       ...
       $('#datatable').dataTable( {
            ...
            "aoColumns": [
                ...
                { 
                    "mDataProp": function ( data, type, full, meta) {
                        return data.appeal_description;
 ...
 $(document).on('change', '#image_url', function() {
    ...
    $(document).on('click', '.btnEdit', function() {
        ...
        mode = "update";
        $('#myModal').modal('show');
        // $('#appeal_title').val(coreJSON[r_index].appeal_title);
        appealdescription.setData(coreJSON[r_index].appeal_description);
        $('#user_uploaded_image').attr("src", base_URL+coreJSON[r_index].image_url);
        $("#user_uploaded_image").addClass("active");
        $("#user_uploaded_image").css("display", "block");
        appeal_id = coreJSON[r_index].appeal_id;
    });
    ...
    $(document).on('click','.BtnDelete',function(){
        ...
        Swal.fire({
            ...
            $.ajax({
                type: "POST",
                url: base_URL+'admin/KaanalAdmin/delete_appeal',
